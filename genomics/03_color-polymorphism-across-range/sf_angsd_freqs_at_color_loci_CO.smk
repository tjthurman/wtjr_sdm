...
rule all:
    input:
        expand("results/angsd_by_region/colorado/{gene}_GL.beagle.gz", gene = genes.keys())
...
rule calc_beagle_GLs_by_scaff:
    input:
        bamlist= "data/bamlists/colorado_samples_bamlist.txt",
        ref=REF
    output:
        "results/angsd_by_region/colorado/{gene}_GL.beagle.gz"
    params:
        basename=lambda wildcards: "results/angsd_by_region/colorado/" + wildcards.gene + "_GL",
        region=lambda wildcards: genes.get(wildcards.gene)
    priority: 10
    log:
        "logs/angsd_by_region/colorado/{gene}.txt"
    resources:
        cpus=18
    shell:
        """
